...
class NewMatchForm(Form):
    ...
    goal = IntegerField(
        "Goal amount (USD, integer)", [validators.DataRequired(), validators.NumberRange(min=10)]
    )


class NewDonationForm(Form):
    name = StringField("Your name (optional)", default="Anonymous", render_kw={"placeholder": "Anonymous"})
